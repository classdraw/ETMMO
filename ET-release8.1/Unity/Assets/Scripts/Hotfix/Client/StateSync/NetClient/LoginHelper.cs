...
namespace ET.Client
{
    ...
    {
        //总登录流程 现在拆解
        public static async ETTask LoginOld(Scene root, string account, string password, string baseExternalDisplay, string name)
        {
            //root是客户端 main fiber 
            ...
            if (r2CGetRoles.RoleInfoList.Count<=0)
            {
                ...
                c2RCreateRole.BaseExternalDisplay = baseExternalDisplay ?? string.Empty;
            ...
            var netClient2MainLoginGame = await clientSenderComponent.LoginGameAsync(
                account, r2CGetRealmKey.Key, roleInfoProto.Id, r2CGetRealmKey.Address, roleInfoProto.BaseExternalDisplay, name);
        ...
        public static async ETTask<LoginOperationResult> LoginCreateRole(Scene root, string roleName, string baseExternalDisplay)
        {
            NetworkCacheComponent cache = root.GetComponent<NetworkCacheComponent>();
            if (cache == null)
            {
        ...
        public static async ETTask<LoginOperationResult> LoginRoleEnterGame(Scene root, long roleId, string baseExternalDisplay, string name)
        {
            NetworkCacheComponent cache = root.GetComponent<NetworkCacheComponent>();
            if (cache == null)
            {
            ...
            //r2CGetRealmKey.Key 是随机64位+时间的hashcode
            var netClient2MainLoginGame = await clientSenderComponent.LoginGameAsync(
                cache.Account, r2CGetRealmKey.Key, roleId, r2CGetRealmKey.Address, baseExternalDisplay, name);
            if (netClient2MainLoginGame==null||netClient2MainLoginGame.Error!=ErrorCode.ERR_Success)
            {
