...
namespace XEngine.Hud{
    class CSerialzieTextStream
    {
        ...
        {
            m_kData = new byte[nBufSize];
            if (buffer != null)
                Array.Copy(buffer, m_kData, nBufSize);
            m_iSize = m_iSizeMax = nBufSize;
            m_iPos = 0;
            m_eSerializeType = arType;
            m_bLittleByte = System.BitConverter.IsLittleEndian;
            m_bEndLine = false;
        ...
        public bool CanRead { get { return m_eSerializeType == SerializeType.read; } }
        public bool CanSeek { get { return true; } }
        public bool CanWrite { get { return m_eSerializeType == SerializeType.read || m_eSerializeType == SerializeType.append; } }
        public void Flush()
        {
            m_iPos = 0;
            m_iSize = 0;
        ...
        {
            if (m_iSizeMax < nSizeMax)
            {
                byte[] pData = new byte[nSizeMax];
                m_iSizeMax = nSizeMax;
                if (m_iSize > 0)
                    Array.Copy(m_kData, pData, m_iSize);
                m_kData = pData;
            }
        }
        void auto_grow(int nGrowSize)
        {
            if (nGrowSize > 0 && m_iPos + nGrowSize > m_iSizeMax)
            {
                int nNewSize = m_iSizeMax * 2 + (nGrowSize + 4095) / 4096 * 4096;
                if (nNewSize < 4096)
                    nNewSize = 4096;
                reserve(nNewSize);
            }
        ...
        string get_temp()
        {
            if (m_iStringLen <= 0)
                return string.Empty;
            while (m_iStringLen > 0 && m_kTempString[m_iStringLen - 1] == 32 )
            {
                --m_iStringLen;
            }
            return System.Text.Encoding.UTF8.GetString(m_kTempString, 0, m_iStringLen);
        }
        void push_temp(byte ch)
        {
            if (m_iStringLen + 1 > m_iStringMaxLen)
            {
                m_iStringMaxLen = m_iStringMaxLen > 0 ? (m_iStringMaxLen * 3 / 2) : 32;
                byte[] pNewString = new byte[m_iStringMaxLen];
                if( m_kTempString != null && m_iStringLen > 0 )
                    Array.Copy(m_kTempString, 0, pNewString, 0, m_iStringLen);
                m_kTempString = pNewString;
            }
            m_kTempString[m_iStringLen] = ch;
            m_iStringLen += 1;
        }
        // 功能：读取头
        ...
        void push_string(string strValue, bool bCheck)
        {
            ...
            Array.Copy(byBuf, 0, m_kData, m_iPos, byBuf.Length);
            m_iPos += byBuf.Length;
            if (m_iSize < m_iPos)
                m_iSize = m_iPos;
        }
        ...
        public void ReadArray(string szName, ref int[] aValue)
        {
            ...
            int nIndex = 0;
            while (!m_bEndLine && m_iPos < m_iSize)
            {
                string szValue = readString();
    ...
    public class SerializeText
    {
        ...
        ~SerializeText()
        {
            if (m_eSerializeType != SerializeType.read && !string.IsNullOrEmpty(m_szFileName))
            {
                Close();
            }
        }
